...
<script>
    $(document).ready(function() {
        $('#categories-table').DataTable({
            ...
            columns: [
                ...
                },
                {
                    data: 'id',
                    render: function(data, type, row) {
                        let editUrl = "{{ route('admin.ticket-categories.edit', ':id') }}".replace(':id', data);
                        let deleteUrl = "{{ route('admin.ticket-categories.destroy', ':id') }}".replace(':id', data);
                        return `
        ...
        $(document).on('click', '.toggle-status', function() {
            ...
            const btn = $(this);
            let toggleUrl = "{{ route('admin.ticket-categories.toggle-status', ':id') }}".replace(':id', id);
            $.post(toggleUrl, {
                _token: '{{ csrf_token() }}'
            }, function(response) {
                if (response.success) {
