            ...
            <div class="space-y-8">
                ...
                <div>
                    ...
                    <textarea name="answer" id="answer" rows="6" placeholder="Provide a detailed and helpful response..." 
                        class="appearance-none block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm transition-colors" required>{{ old('answer', $faq->answer) }}</textarea>
                    @error('answer')
                        <p class="mt-2 text-xs font-bold text-red-500">{{ $message }}</p>
                    @enderror
                </div>
