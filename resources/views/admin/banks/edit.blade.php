...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-black text-gray-900 dark:text-white mt-4 tracking-tight">Edit Bank Record</h1>
        <p class="mt-2 text-sm font-medium text-gray-500 dark:text-gray-400 shadow-sm leading-relaxed">Update the official information for <span class="text-amber-600 dark:text-amber-500 font-bold">{{ $bank->name }}</span>.</p>
    </div>
    ...
    <div class="bg-white dark:bg-richdark-surface rounded-3xl shadow-xl border border-gray-200 dark:border-white/5 overflow-hidden transition-all duration-300">
        <form action="{{ route('banks.update', $bank) }}" method="POST" class="p-8 md:p-12">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-8">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $bank->name) }}" placeholder="e.g. State Bank of India" required 
                ...
                <div class="pt-6 flex justify-end">
                    <button type="submit" class="inline-flex items-center justify-center px-8 py-3.5 border border-transparent text-sm font-black rounded-2xl shadow-xl text-white bg-amber-600 hover:bg-amber-700 focus:outline-none focus:ring-4 focus:ring-amber-500/40 transition-all transform hover:scale-[1.02] active:scale-[0.98]">
                        Update Bank Record <i class="fas fa-check-circle ml-3 text-sm opacity-80"></i>
                    </button>
                </div>
            </div>
        </form>
