...
<div class="mb-6 flex justify-between items-center">
    ...
    <a href="{{ route('cities.index') }}" class="text-amber-600 hover:text-amber-800 text-sm font-medium flex items-center bg-white px-3 py-1.5 rounded border border-gray-200 shadow-sm">
        ...
        <form action="{{ route('cities.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="col-span-1 md:col-span-2 lg:col-span-3">
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name') }}" class="mt-1 focus:ring-amber-500 focus:border-amber-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border" placeholder="Enter city name">
                ...
                <div>
                    ...
                    <input type="text" name="type" id="type" value="{{ old('type') }}" class="mt-1 focus:ring-amber-500 focus:border-amber-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border" placeholder="e.g. city">
                ...
                <div>
                    ...
                    <select name="timezone_id" id="timezone_id" class="mt-1 focus:ring-amber-500 focus:border-amber-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border">
                        ...
                        @foreach($timezones as $tz)
                            <option value="{{ $tz->id }}" {{ old('timezone_id') == $tz->id ? 'selected' : '' }}>{{ $tz->zone_name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mt-8 border-t border-gray-200 pt-5">
                <div class="flex justify-end">
                    <a href="{{ route('cities.index') }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
                        Cancel
                    </a>
