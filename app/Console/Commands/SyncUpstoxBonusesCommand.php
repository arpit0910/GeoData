...
class SyncUpstoxBonusesCommand extends Command
{
    protected $signature = 'market:sync-upstox-bonuses
        {--limit=25 : Number of equities to check per run}
        {--isin=* : Specific ISINs to sync}';

    protected $description = 'Sync bonus issues separately from Upstox';
    ...
    public function handle(UpstoxCorporateActionSyncService $syncService): int
    {
        ...
        $this->info("Checking bonus share issues for {$equities->count()} equities...");
        $stats = $syncService->syncForEquities($equities, ['BONUS']);

        $this->info("Bonus sync complete: {$stats['saved']} bonus records saved/updated.");
        return self::SUCCESS;
    }
}
